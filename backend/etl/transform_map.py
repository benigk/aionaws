...
def usa_states_maker():
    """making the usa states mapping"""
    us_state_abbrev = {
        'AL': 'Alabama',
        'AK': 'Alaska',
        'AZ': 'Arizona',
...
def spark_map(input_s, S):
    ...
    country_list = country_list_maker()
    usa_list = usa_states_maker()
    # S = 'hh'
    if len(input_s["gepEntities"]) == 0:
        return None, None
    elif len(input_s["gepEntities"]) == 1:
        ...
        if input_s["gepEntities"][-1] in country_list:
            if country_list[input_s["gepEntities"][-1]] == 'USA' and input_s["gepEntities"][-2] in usa_list:
                return input_s['gepEntities'], [usa_list[input_s["gepEntities"][-2]], 'USA']
            else:
                city = re.sub(r'[0-9]+', '',
                ...
                return input_s['gepEntities'], tmp_res
        elif input_s["gepEntities"][-1] in usa_list:
            # tmp_res = get_place_res(input_s["gepEntities"][-2], 'USA')
            return input_s['gepEntities'], [usa_list[input_s["gepEntities"][-1]], 'USA']
        else:
            tmp_res = get_place_res(input_s["gepEntities"][-1], '', S)
            return input_s['gepEntities'], tmp_res
...
def partitins_map(iterator):
    ...
    kwargs = dict(hosts=['localhost'], port=9200, use_ssl=False)
...
# final_df  = spark.createDataFrame(final_df,schema=gep_type)

# final_df.write.format('json') \
#     .mode('overwrite') \
#     .save("./gep_test")
# final_df.repartition(4)
# test_dict = {
#     "gepEntities": ["KU Leuven", "Leuven", "Belgium"],
# }

# print(spark_map(test_dict))
